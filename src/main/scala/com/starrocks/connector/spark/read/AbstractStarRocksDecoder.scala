...
import com.starrocks.connector.spark.rdd.{BaseValueReader, BypassValueReader, RpcValueReader}
...
abstract class AbstractStarRocksDecoder[Record <: StarRocksGenericRow](partition: InputPartition,
  ...
  lazy val reader: BaseValueReader = {
    val baseValueReader = if (ConfigUtils.isBypassRead(settings)) {
      new BypassValueReader(partition.asInstanceOf[BypassPartition], settings, schema)
    } else {
      new RpcValueReader(partition.asInstanceOf[RpcPartition], settings)
    }
    baseValueReader.init()
    baseValueReader
  }
  var rowOrder: Seq[String] = if (ConfigUtils.isBypassRead(settings)) {
