...
class ReplaceAggregator extends SparkRDDAggregator<Object> {
    ...
    @Override
    Object update(Object culVal, Object newVal) {
        return newVal;
    }
}
...
class ReplaceIfNotNullAggregator extends SparkRDDAggregator<Object> {
    ...
    @Override
    Object update(Object curVal, Object newVal) {
        return newVal == null ? curVal : newVal;
    }
}
...
class BucketPartitioner extends Partitioner {
    ...
    public int getPartition(Object key) {
        if (null == key) {
            throw new IllegalStateException("Null key when get partition.");
        }

        String bucketKey = String.valueOf(((List<Object>) key).get(0));
        Integer partitionId = bucketKeyMap.get(bucketKey);
        if (null == partitionId) {
            throw new IllegalStateException("no partition found for " + bucketKey);
        }
        return partitionId;
    }
}