...
public class BaseRowBatch implements AutoCloseable {
    ...
    protected void addValueToRow(long rowIndex, Object value) {
        if (rowIndex > rowCountInOneBatch) {
            throw new IllegalStateException(
                    String.format("row offset %d is larger than row size %d", rowIndex, rowCountInOneBatch));
        }
        long idx = readRowCount + rowIndex;
        if (idx < 0 || idx > Integer.MAX_VALUE) {
            throw new IllegalStateException("invalid row idx: " + idx);
        }
        rowBatch.get((int) idx).put(value);
    }
